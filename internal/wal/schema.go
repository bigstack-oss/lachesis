...
type flowMetricsWire struct {
	...
	// CreatedNs (v7+) is the kernel entry-identity stamp. Absent from a
	// v6 file, where it decodes as 0 — which ApplyDelta reads as
	// "identity unknown" and handles with the value guard, NOT as a
	// reset (that would re-count a pinned map's whole cumulative).
	CreatedNs uint64 `json:"created_ns,string,omitempty"`
}
