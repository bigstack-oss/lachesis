...
// per cold-start (and per future incremental resync) so the
// /debug landing page can surface misconfigurations before they
// corrupt billing.
//
// The four classes are independent — a single misconfigured router
// can appear in more than one slice (e.g. a dangling extraroute on
// a router that also participates in a cycle).
...
func detectDuplicateRouterMACs(snap Snapshot) []DuplicateRouterMAC {
	...
	for _, p := range snap.Ports {
		if p.DeviceOwner != "network:router_interface" || p.MACAddress == "" {
			continue
		}
