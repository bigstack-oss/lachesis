...
func runScenario(snap neutron.Snapshot) []neutron.TrieEntry {
	entries, _, _ := neutron.BuildTrie(snap.Networks, snap.Subnets, snap.Ports, snap.Routers)
	return entries
}
...
func TestBuildTrie_SurfacesAmbiguityHit(t *testing.T) {
	...
	entries, hits, _ := neutron.BuildTrie(snap.Networks, snap.Subnets, snap.Ports, snap.Routers)
