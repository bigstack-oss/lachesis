...
// EXTERNAL for each — but [DetectAnomalies] surfaces them on the
// /debug landing page so an operator can fix the underlying
// misconfiguration.
//
// # Step coverage
//
...
func BuildTrie(networks []Network, subnets []Subnet, ports []Port, routers []Router, opts ...BuildOpt) ([]TrieEntry, []AmbiguityHit, []CycleHit) {
	var bo buildOpts
	for _, o := range opts {
		o(&bo)
	}
