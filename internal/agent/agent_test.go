...
func TestAgent_ServesMetricsFromReader(t *testing.T) {
	...
	body := mustGetMetrics(t, ag.Addr(), time.Second, func(s string) bool {
		return strings.Contains(s, `lachesis_tenant_bytes_total{direction="rx",external_network="none",tenant_id="unknown",zone="external"} 4242`)
	})

	for _, want := range []string{
		`lachesis_tenant_bytes_total{direction="rx",external_network="none",tenant_id="unknown",zone="external"} 4242`,
		`lachesis_tenant_packets_total{direction="rx",external_network="none",tenant_id="unknown",zone="external"} 7`,
...
func TestAgent_SeedStateAppearsOnMetrics(t *testing.T) {
	...
	body := mustGetMetrics(t, ag.Addr(), time.Second, func(s string) bool {
		return strings.Contains(s, `lachesis_tenant_bytes_total{direction="rx",external_network="none",tenant_id="unknown",zone="external"} 7777`)
	})
	if !strings.Contains(body, `lachesis_tenant_packets_total{direction="rx",external_network="none",tenant_id="unknown",zone="external"} 13`) {
