...
**Agent-config changes are always put back.** A step may retune the agent under test (`RestartAgentStep`/`ReloadAgentStep` with `SetConfig` — how the max-hops scenarios impose a small hop limit and the late-bind scenario suppresses metadata). The step that takes the config backup records the debt, and `run` restores every outstanding one as it exits — on the abort paths too, so a step error, a failed SSH, or a Ctrl-C cannot leave a host serving a scenario's temporary config into whatever runs next. A scenario's own trailing `RestartAgentStep{RestoreConfig: true}` clears the debt, so the end-of-run sweep is a no-op on the happy path; keep declaring it, because it restores at the right point in the script rather than after the last step. The sweep is best-effort and logs at error level if it cannot reach the host — it never masks the failure that caused the abort. Only a hard kill (SIGKILL) escapes it; recover with `cp <config>.scenariotest.bak <config>` and restart the unit.
...
**Known-infeasible at this tier — live per-port project reassignment.** A port's tenant attribution keys on its Neutron `project_id`, which is immutable — no API operation moves a live port to another project — so a literal "port reassignment" cannot be staged. Of the three attribution axes a reconcile `SettleRebase` folds on (tenant / external-network / server), only the last two have any live mutation, and on a single-router OVN topology neither is independent of a router move: a floating IP can only bind to an external network its port's subnet can reach *through a gatewayed router*, so changing a port's external-network attribution reduces to re-gatewaying its router (the **router-regateway** scenario, external-network axis), and changing its server attribution is the cross-server **port-move** scenario (`port-move-across-servers`, server axis). Both fold through the same reconcile path this "reassignment" would; the per-MAC `settleAttributionChange` write-back that prevents a double-count is additionally unit-tested (`internal/reconcile`). This is the documented mapping the Feature #166 / task #190 DoD asks for in place of a distinct project-move scenario.
**Known-infeasible at this tier — pressure-relief GC.** The `telemetry_map` high/low watermark eviction cycle cannot be exercised live. The map is a compile-time `65536`-entry `PERCPU_HASH` (`bpf/telemetry.c`, mirrored and boot-validated by `bpf.ValidateMapSizes`), so it cannot be shrunk through an agent config; and tripping even a lowered `pressure_high_watermark` requires filling that many *distinct* flow keys, i.e. hundreds-to-thousands of distinct `(src_mac, dst_mac, zone)` tuples — every external egress from one VM collapses to a single key (peer = its router), so reaching the watermark needs a comparable number of live VM↔peer pairs, far past what a staging cluster can stand up and drive. The eviction logic (flush-before-evict, watermark hysteresis, per-pass cap) is therefore **owned by the unit tier** (`internal/gc` pressure-relief tests, table-driven over synthetic fill), and the map-full silent-byte-loss boundary by the **integration tier** (a purpose-built small-map binary under BPF caps). This is the written rationale the Feature #166 DoD asks for in lieu of a live probe.
