...
- Health metrics: `cubecos_unresolved_buffer_depth`, `cubecos_unresolved_buffer_evictions_total{reason="lru|expired"}`, `cubecos_unresolved_resolved_total`, `cubecos_gc_evictions_total{reason="ttl|pressure_relief"}`, `cubecos_lingering_ghosts_active`, `cubecos_gc_pressure_relief_runs_total`. The GC's trigger reads the same telemetry-map entry count already exported as `cubecos_bpf_map_current_entries{map="telemetry_map"}` (landed with the metrics gap-fill alongside `cubecos_collect_duration_seconds`; see §11.4).
...
- Kafka health metrics: `cubecos_kafka_lag_messages{topic}`, `cubecos_kafka_consume_errors_total{topic}` — the dedicated counter is the error sink (§11.4 dropped the generic `cubecos_internal_errors_total`). Runtime kernelwriter failures on the incremental path also need a dedicated counter here — the first case §11.4's revisit note anticipates.
