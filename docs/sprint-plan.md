...
**Status update (2026-06-11, PR #26):** the per-topic dashboard files were consolidated into one all-in-one dashboard, `deploy/grafana/dashboards/lachesis-telemetry.json` (~32 panels: overview, throughput, BPF maps, scraper/collector, Neutron incl. anomalies, WAL, netlink/TC). The table above remains the panel roadmap; new panels land in that single file.
...
- Health metrics from §11.4: `lachesis_neutron_sync_age_seconds`, `lachesis_neutron_api_errors_total{endpoint, code}`, `lachesis_bpf_map_fill_ratio{map="trie|mac_tenant"}`, `lachesis_bpf_map_max_entries{map=...}`.
...
- Agent boots against the real 3-chassis OVN Yoga cluster; `/metrics` shows non-empty `lachesis_bytes_total{tenant_id=<real-UUID>,...}` series (no `unknown` for tenants with active VMs).
- `lachesis_neutron_sync_age_seconds` < 60 in steady state.
- Stopping the Neutron API blocks boot with backoff (verified by log + `lachesis_neutron_api_errors_total`); recovery completes once the API returns.
...
- `lachesis_neutron_builder_step_duration_seconds` histogram (per-step timing, useful when route resolution is slow on big deployments).
...
- Health metrics: `lachesis_unresolved_buffer_depth`, `lachesis_unresolved_buffer_evictions_total{reason="lru|expired"}`, `lachesis_unresolved_resolved_total`, `lachesis_gc_evictions_total{reason="ttl|pressure_relief"}`, `lachesis_lingering_ghosts_active`, `lachesis_gc_pressure_relief_runs_total`. The GC's trigger reads the same telemetry-map entry count already exported as `lachesis_bpf_map_current_entries{map="telemetry_map"}` (landed with the metrics gap-fill alongside `lachesis_collect_duration_seconds`; see §11.4).
...
- Kafka health metrics: `lachesis_kafka_lag_messages{topic}`, `lachesis_kafka_consume_errors_total{topic}` — the dedicated counter is the error sink (§11.4 dropped the generic `lachesis_internal_errors_total`). Runtime kernelwriter failures on the incremental path also need a dedicated counter here — the first case §11.4's revisit note anticipates.

**Done when.** Recorded Kafka stream replay against a known-state cluster → trie matches a fresh cold-start at the same point in time. Kill Kafka for >5 minutes → `lachesis_neutron_sync_age_seconds` resets to <300 after each reconcile pass; Kafka resumed → trie converges within one consume cycle.

**LOC.** ~600 (originally ~500; +100 for the 5-minute reconcile + per-endpoint failure isolation).

---

> **Re-sequencing (2026-06-23).** Before Octavia, the next delivered sprint is the **live scenario-validation harness (`scenariotest`)** plus **netlink attach-presence robustness** (a present-but-unattached gauge and the periodic re-attach reconciler — un-deferring DESIGN §13.2 #5). The harness automates the manual deploy→traffic→scrape→assert→teardown loop and de-risks Octavia; the netlink work closes a silent-billing-loss gap (missed NEWLINK events under churn are invisible to `lachesis_tc_attach_failures_total`). Tracked on the board (cubecos #984 feature + tasks #985–#990; netlink #992/#993; Kafka-debounce #991 backlogged). Octavia, Hardening, and IPv6 each shift one slot; the sprint numbers below are intentionally **not** renumbered, to avoid churning the §-cross-references throughout this doc.
>
...
| 2026-06-11 | #26 | Metrics gap-fill (telemetry_map fill gauges, `lachesis_collect_duration_seconds`, DESIGN §11.4 realign) + all-in-one Grafana dashboard |
| 2026-06-11 | #27 | `/debug` operator surface (`internal/debug`): index, anomalies, lookup, zones, topology pages + pprof; `lachesis_neutron_anomalies{class}` gauge |
