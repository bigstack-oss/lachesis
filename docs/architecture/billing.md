...
- **Mortal series.** Unlike the tenant family, a per-server series *ends* when its attribution dies (VM deleted and ghost-swept, and its carry seed then expires). While the server is alive its bytes are held monotone by a per-server **carry** bucket — the mortal-family fold absorber, keyed by the full server tuple `{server_id, tenant_id, zone, external_network, direction}` — so a partial fold on a multi-port server or a same-server port recreate never makes the still-live series dip (lachesis#226/#227). Once the server is fully dead the carry lies dormant as a same-window rebirth seed and is dropped after the carry TTL (`gc.server_carry_ttl`, hot-reloadable), so dead servers accumulate no unbounded state. Dead-server history is then the scraping TSDB's responsibility; its retention must exceed the consumer's maximum tolerable ETL outage (90 days on the target platform, far above the few days actually required). A mortal series is PromQL-safe: it only ever *stops*, it never *decreases* — the poison the carry exists to prevent is a *continuing* series that drops.
- **Compaction floor while alive:** no agent-side compaction folds a live server's bytes past the server key — the per-live-server carry bucket IS that floor, and its set is bounded by live + within-TTL servers ([data-structures.md](./data-structures.md#settled-bytes)). Tenant-level settle happens in parallel at attribution death (the two accumulators are independent). Without the carry the exposed per-server series would decrease and break subtraction at server granularity.
