...
`lachesis_server_bytes_total{server_id, port_id, tenant_id, zone, external_network, direction}` — cumulative, emitted from the same GlobalState (WAL-backed, restart-surviving) as the tenant family, but broken out **per port**: each Neutron port is its own mortal series and the consumer aggregates to the billable `server_id`. `server_id` is the port `device_id` (Nova instance UUID), stable across live migration; `port_id` is the Neutron port UUID — below the billing granularity (the invoice is per server), a monitoring/drill-down dimension that also makes the family monotone-safe without any agent-side aggregation; `user` is not emitted (Neutron ports don't carry it) — the billing consumer derives ownership from `server_id`.
...
- **Mortal, per port.** Each per-port series *ends* when its port is deleted (and ghost-swept) — it simply **stops**, which is PromQL-safe. There is deliberately **no agent-side per-server aggregation and no carry**: a port that dies leaves nothing to dip, and a recreated port is a new series (new `port_id`). Dead-port history is the scraping TSDB's responsibility; its retention must exceed the consumer's maximum tolerable ETL outage (90 days on the target platform, far above the few days actually required). The invariant is that a mortal series only ever *stops*, never *decreases* — a *continuing* series that drops is the poison, and emitting **per port** rather than a pre-summed per-server aggregate avoids it at the source: no aggregate exists to drop when one port folds.
- **Consumption contract (the billing ETL) — Δ-per-series then sum:** read a day's range **per series** and take boundary values — present at both ends: `Δ = last − first`; born mid-window: `Δ = last` (a counter born in-window carries its lifetime bytes); died mid-window: `Δ = last sample − first`; negative `Δ` clamps to 0 (a hard crash can regress a counter by up to the WAL flush window — provider-unfavorable, self-correcting). The per-server total is then `sum by (server_id)` of those per-series `Δ`s — summing across a server's **ports** and across scrape **instances** (a live-migrated server appears on several). **Never a naive `sum by (server_id)` *then* subtract** (the summed series drops the instant any port stops — the poison, relocated into the query), and **never `increase()`/`rate()` for money** (PromQL's reset heuristic double-counts a crash regression). Dashboards may use `sum by (server_id)(rate(...))` — per-series rate first, then sum, the kube-pod pattern.
