...
| `lachesis_scrape_duration_seconds` | histogram | — | one **successful** scraper tick: `telemetry_map` BatchLookup drain + delta integration + pressure-relief pass (failed drains excluded — they exit early and would drag the quantiles down; see `lachesis_scraper_errors_total`). The write-side cost, growing with `entries × N_CPU` ([performance.md](./performance.md)) — distinct from `lachesis_collect_duration_seconds`, which times the read/exposition path. Buckets 1ms..1s |
