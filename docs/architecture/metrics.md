...
| `lachesis_scrape_duration_seconds` | histogram | — | one scraper tick: `telemetry_map` BatchLookup drain + delta integration + pressure-relief pass. The write-side cost, growing with `entries × N_CPU` ([performance.md](./performance.md)) — distinct from `lachesis_collect_duration_seconds`, which times the read/exposition path. Buckets 1ms..1s |
