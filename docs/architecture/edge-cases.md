...
| 17 | Octavia base address missing from `amphora_base_ip` (Octavia list stale, Amphora still booting) | That flow's Segment 2 zones `same_tenant` instead of `infra` | No billing impact — both are $0 — and it self-corrects on the next reconcile. Fails in this direction by design: the set holds base addresses rather than VIPs precisely so a gap cannot mark Segment 1 `infra` and stop billing it. See [octavia.md](./octavia.md) |
