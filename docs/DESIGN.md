...
- **External-network attribution rules.** A VM's `external_network` is its floating IP's network, else the external gateway of the router owning its subnet's `gateway_ip` — the *gateway-IP rule*: several routers may attach to one subnet, but the VM's default route points only at the gateway owner, so SNAT egress is deterministic (non-gateway routers count only when no gateway owner has an external gateway — the in-guest static-route case, §8 Tier 4 #9). A VM with genuinely multiple external paths (FIPs from different external networks) is attributed to ONE, picked deterministically (lexicographically smallest label, FIP tier first) so reconcile passes never flap the label; every such VM is surfaced on `lachesis_neutron_anomalies{class="multi_external_path"}` with per-port drill-down at `/debug/anomalies` — its per-network external split is approximate until the limitation is lifted. The `multi-external-path` scenariotest scenario is the live regression for all three behaviors.
